...
import * as d3Module from "d3-color-difference"
import { cubehelix, lab, rgb } from "d3-color"
import { scaleLinear } from "d3-scale"

console.log(d3Module)
const differenceEuclideanLab = () => true
...
export function getColorData(value, toneFn, accentFn) {
	const altColor = (_.isObject(value) && value.__altViaOpacity) || null
	const color = rgb(value)
	const accentsIsNeeded = _.isFunction(accentFn)
	const tonesIsNeeded = _.isFunction(toneFn)
	...
	if (!_.isNull(altColor) && altColor.opacity < 1) {
	...
	const meta = accentsIsNeeded
		? ((diffs) => {
			...
			const minEntry = _.minBy(entries, e => e[1])
			return {
