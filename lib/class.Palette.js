...
class Palette {
	...
	get flatten() {
		return (
			...
			(() => {
				...
				/**
				 * @param {PrimaryColor} primary
				 * @param {PrimaryKey} PK
				 * @returns {void}
				 */
				const flatten = (primary, PK) => {
					const { accents, shades } = primary
					// const primary = omit(color, 'accents', 'shades')

					this.#flatCache.set(PK, primary)

					shades.forEach((shade, TK) =>
						this.#flatCache.set(`${PK}-${TK}`, shade)
					)
					accents?.forEach((accent, AK) =>
						this.#flatCache.set(`${PK}-a${AK}`, accent)
					)
				}
	...
	get background() {
		return this.#background.to(this.#config.space.input)
	}

	get foreground() {
		return this.#foreground.to(this.#config.space.input)
	}
