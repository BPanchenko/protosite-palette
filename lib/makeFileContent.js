 ...
 * @param {Array<[import("../").ColorName, import("../").ColorValue]>} data
...
export const makeCssContent = (data) => {
	const str = `:root {${data.map(([name, value]) => `--clr-${kebabCase(name).replace('a-', 'a')}: ${value};`).join('')}}`;
	return str2ab(str);
};

/** @type {MakeFileContent} */
export const makeDtsContent = (data) => {
	const str = data.map(([name]) => `export const clr${startCase(name)}: string;`).join('');
	return str2ab(str);
};

/** @type {MakeFileContent} */
export const makeEsmContent = (data) => {
	const str = data.map(([name, value]) => `export const clr${startCase(name)} = '${value}';`).join('');
	return str2ab(str);
};
